...
public class LemmyPersonMentionService {
    ...
    public PersonMentionView getPersonMentionView(PersonMention personMention) {
        ...
        return PersonMentionView.builder().person_mention(conversionService.convert(personMention, com.sublinks.sublinksapi.api.lemmy.v3.user.models.PersonMention.class)).counts(conversionService.convert(commentAggregates, CommentAggregates.class)).creator(conversionService.convert(creator, com.sublinks.sublinksapi.api.lemmy.v3.user.models.Person.class)).creator_blocked(false).comment(conversionService.convert(comment, com.sublinks.sublinksapi.api.lemmy.v3.comment.models.Comment.class)).post(conversionService.convert(comment.getPost(), com.sublinks.sublinksapi.api.lemmy.v3.post.models.Post.class)).saved(false).recipient(conversionService.convert(personMention.getRecipient(), com.sublinks.sublinksapi.api.lemmy.v3.user.models.Person.class)).community(conversionService.convert(comment.getCommunity(), com.sublinks.sublinksapi.api.lemmy.v3.community.models.Community.class))

                .my_vote(commentLike == null ? 0 : commentLike.getScore()).creator_banned_from_community(false).subscribed(lemmyCommunityService.getPersonCommunitySubscribeType(creator, comment.getCommunity())).build();
    }
}
