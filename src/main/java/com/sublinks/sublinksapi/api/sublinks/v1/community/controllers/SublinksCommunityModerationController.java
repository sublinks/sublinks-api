...
public class SublinksCommunityModerationController extends AbstractSublinksApiController {
    ...
    public CommunityResponse delete(@PathVariable final String key, @RequestBody @Valid CommunityDelete communityDeleteForm, SublinksJwtPerson sublinksJwtPerson) {
    ...
    @GetMapping("/remove")
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "OK", useReturnTypeSchema = true)})
    public CommunityResponse remove(@PathVariable final String key, @RequestBody @Valid CommunityRemove communityRemoveForm, SublinksJwtPerson sublinksJwtPerson) {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "OK", useReturnTypeSchema = true)})
    public List<CommunityModeratorResponse> show(@PathVariable final String key, final SublinksJwtPerson sublinksJwtPerson) {

        Optional<Person> person = getOptionalPerson(sublinksJwtPerson);

        rolePermissionService.isPermitted(person.orElse(null), RolePermissionCommunityTypes.READ_COMMUNITY_MODERATORS, () -> {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "not_authorized_to_read_moderators");
        });

        return sublinksCommunityService.getCommunityModerators(key, person.orElse(null))
                                       .stream()
                                       .map(communityModerator -> conversionService.convert(communityModerator, CommunityModeratorResponse.class))
                                       .toList();
    }
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "OK", useReturnTypeSchema = true)})
    public List<CommunityModeratorResponse> add(@PathVariable final String key, @PathVariable final String personKey, final SublinksJwtPerson sublinksJwtPerson) {

        final Person person = getPersonOrThrowUnauthorized(sublinksJwtPerson);

        final Community community = communityRepository.findCommunityByTitleSlug(key)
                                                       .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "community_not_found"));

        if (!(linkPersonCommunityService.hasAnyLinkOrAdmin(person, community, List.of(LinkPersonCommunityType.moderator, LinkPersonCommunityType.owner)) && rolePermissionService.isPermitted(person, RolePermissionCommunityTypes.MODERATOR_ADD_MODERATOR)) && !rolePermissionService.isPermitted(person, RolePermissionCommunityTypes.ADMIN_ADD_COMMUNITY_MODERATOR)) {
        ...
        if (linkPersonCommunityService.hasAnyLinkOrAdmin(newModerator, community, List.of(LinkPersonCommunityType.moderator, LinkPersonCommunityType.owner))) {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "OK", useReturnTypeSchema = true)})
    public List<CommunityModeratorResponse> remove(@PathVariable final String key, @PathVariable final String personKey) {
        ...
        if (!linkPersonCommunityService.hasAnyLinkOrAdmin(person, community, List.of(LinkPersonCommunityType.moderator, LinkPersonCommunityType.owner))) {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "OK", useReturnTypeSchema = true)})
    public PersonResponse ban(@PathVariable final String key, @PathVariable final String personKey, CommunityBanPerson communityBanPersonForm, final SublinksJwtPerson sublinksJwtPerson) {
        ...
        final Person bannedPerson = sublinksCommunityService.banPerson(key, personKey, person, communityBanPersonForm);
    ...
    public List<PersonResponse> banned(@PathVariable final String key) {
        ...
        return linkPersonCommunityService.getPersonsFromCommunityAndListTypes(community, List.of(LinkPersonCommunityType.banned))
                                         .stream()
