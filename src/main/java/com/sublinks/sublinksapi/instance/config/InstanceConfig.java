...
public class InstanceConfig {
  ...
  public InstanceConfig(
      @Value("${sublinks.settings.local_instance_id:1}") final Long localInstanceId) {
  ...
  InstanceAggregate localInstanceAggregate(final InstanceAggregateRepository aggregateRepository) {
    ...
    final Optional<InstanceAggregate> localInstanceAggregate = aggregateRepository.findById(
        localInstanceId);
