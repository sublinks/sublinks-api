...
public interface CommentSearchRepository extends JpaRepository<Comment, Long> {
   ...
   //        nativeQuery = true)
   @Query(value = "SELECT c.*, p.* FROM comments c LEFT JOIN posts p on p.id = c.post_id LEFT JOIN people pp ON pp.id = c.person_id LEFT JOIN communities cm ON cm.id = c.community_id WHERE MATCH(p.commentBody) AGAINST (CONCAT('*', :keyword, '*') IN BOOLEAN MODE);",
           nativeQuery = true)
